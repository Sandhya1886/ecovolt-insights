...
const steps = [
  { num: "01", title: "Collect & Classify", desc: "IoT sensors and AI classify waste at source automatically", icon: Recycle, link: "/classification" },
  { num: "02", title: "Analyze & Predict", desc: "ML models analyze waste streams and predict energy potential", icon: TrendingUp, link: "/energy-prediction" },
  { num: "03", title: "Optimize & Convert", desc: "Smart routing and conversion maximize energy recovery", icon: Zap, link: "/route-optimization" },
];
    ...
    <div className="overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-primary text-sm font-medium mb-8">
              <Leaf className="w-4 h-4" /> AI-Powered Waste Intelligence
            </div>
            <h1 className="font-display text-5xl sm:text-6xl md:text-7xl font-bold leading-tight mb-6">
              Transform Waste Into{" "}<span className="gradient-text">Energy Intelligence</span>
            </h1>
            <p className="text-lg sm:text-xl text-muted-foreground max-w-2xl mx-auto mb-10">
              EcoVolt combines AI, IoT, and data analytics to optimize waste collection, predict energy generation, and build sustainable smart cities.
            </p>
      ...
      <section className="py-24">
        <div className="max-w-6xl mx-auto px-4">
          <AnimatedSection className="text-center mb-16">
            <h2 className="font-display text-3xl sm:text-4xl font-bold mb-4">Intelligent Waste Management</h2>
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">Six powerful AI modules working together to transform how cities handle waste</p>
          </AnimatedSection>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((f, i) => (
              <AnimatedSection key={i}>
                <motion.div whileHover={{ y: -4, scale: 1.02 }} className="glass-card p-6 h-full hover:glow-border transition-shadow duration-300">
              ...
              <AnimatedSection key={i}>
                <Link to={step.link} className="block group">
                  <div className="text-center">
                    <div className="w-20 h-20 rounded-2xl glass-card glow-border flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform">
                      <step.icon className="w-8 h-8 text-primary" />
                    </div>
                    <span className="text-primary font-display font-bold text-sm">{step.num}</span>
                    <h3 className="font-display text-xl font-semibold mt-2 mb-3 text-foreground group-hover:text-primary transition-colors">{step.title}</h3>
      ...
      <section className="py-24">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <AnimatedSection>
            <h2 className="font-display text-3xl sm:text-4xl font-bold mb-6">Ready to Power the Future with Waste?</h2>
            <p className="text-muted-foreground text-lg mb-10 max-w-xl mx-auto">Join smart cities worldwide using EcoVolt to turn waste into clean energy.</p>
            <Link to="/login">
      ...
      <footer className="border-t border-border/50 py-12">
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <p className="text-muted-foreground text-sm text-center">Building a sustainable future through intelligent waste-to-energy solutions.</p>
...
function StatCard({ value, suffix, label, decimals, delay }: { value: number; suffix: string; label: string; decimals: number; delay: number }) {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, scale: 0.9 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.5, delay }} className="text-center">
      <div className="font-display text-4xl sm:text-5xl font-bold text-primary mb-2">
        {inView && <CountUp end={value} duration={2.5} decimals={decimals} />}{suffix}
      </div>
